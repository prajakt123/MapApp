...
function successcallback(position) {
    ...
    var radius = frmOptions.txtDistance.text;
    openNow = " ";
    if (radius == "" || radius == null) radius = "5000";
    if (tmp1 == 0) openNow = " ";
    else openNow = true;
    ...
    //atmUrl="s.googleapis.com/maps/api/place/nearbysearch/json?location=-33.8670522,151.1957362&radius=500&types=atm&sensor=false&key=AIzaSyDg1m_croyAxo67tRJSEEClBIwDUM_qUio";
    try {
        var myhttpheaders = {
            authkey: "myauthkey",
            authtoken: "myauthtoken"
        };
        // var inputParamTable={appID:"Map",serviceID:"accountLogin",channel:"rc",httpheaders:myhttpheaders};//iphone
        var inputParamTable = {};
        inputParamTable["appID"] = "Map";
        //inputParamTable["serviceID"]="accountLogin";
        inputParamTable["channel"] = "rc";
        inputParamTable["httpheaders"] = myhttpheaders;
        inputParamTable["httpconfig"] = {
            method: "get"
        };
...
function callbackfunction(status, resulttable) {
    if (status == 400) {
        ...
        for (i = 0; i < len; i++) {
            ...
            location = {
                "lat": resulttable["results"][i]["geometry"]["location"]["lat"].toString(),
                ...
                "calloutData": {
                    "key1": resulttable["results"][i]["name"].toString(),
                    "key2": resulttable["results"][i]["vicinity"].toString(),
                    template: hbxCallout
                },
                "showcallout": true
            };
        ...
        locationList.push(location);
        frmMap.mapAtms.widgetDataMapForCallout = {
            lblName: "key1",
            lblAddress: "key2"
        };
...
function setViewMode(radio) {
    ...
    switch (id) {
    case "1":
        frmMap.mapAtms.mode = constants.MAP_VIEW_MODE_HYBRID;
        break;
    case "2":
        frmMap.mapAtms.mode = constants.MAP_VIEW_MODE_SATELLITE;
        break;
    case "3":
        frmMap.mapAtms.mode = constants.MAP_VIEW_MODE_NORMAL;
    }
